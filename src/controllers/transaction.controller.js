...
exports.createTransaction = async (req, res) => {
  ...
  if (!service_code) {
    return res
      .status(400)
      .json({ status: false, message: "Service code tidak boleh kosong" });
  }
  ...
  try {
    ...
    if (serviceResult.rowCount === 0) {
      return res
        .status(404)
        .json({ status: false, message: "Service tidak ditemukan" });
    }
    ...
    if (balanceResult.rowCount === 0) {
      return res
        .status(404)
        .json({ status: false, message: "User tidak ditemukan di balances" });
    }
    ...
    if (currentBalance < price) {
      return res
        .status(400)
        .json({ status: false, message: "Saldo tidak mencukupi" });
    }
